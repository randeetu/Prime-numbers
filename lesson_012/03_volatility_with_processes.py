...
class TickerVolatility(Process):
    ...
    def run(self):
        ...
        self.transmeter.put([secid, volatility])
        # self.transmeter.close()
...
def main():
    ...
    transmeter = Queue()
    arranges = [TickerVolatility(path_file=file, transmeter=transmeter) for file in tickers_path]
    ...
    while not transmeter.empty():
        name, vol = transmeter.get()
        if vol == 0:  # если нулевая
            zero_secid[name] = 0  # сохраняем в словарь нулевых волатильностей
        else:  # если не нулевая
            not_zero_secid[name] = round(vol, 2)  # сохраняем в словарь не нулевых волатильностей
