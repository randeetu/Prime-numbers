...
for product_name, product_code in goods.items():
    quantity_all = 0
    cost_all = 0
    for quantity_store in store[product_code]:
        quantity = quantity_store['quantity']
        price = quantity_store['price']
        quantity_all += quantity
        cost = quantity * price
        cost_all += cost
    print(product_name, '-', quantity_all, 'шт, стоимость ', cost_all, 'руб')
